  ...
  function addResultIfTokenMatch(fnode, path, state) {
    ...
    if (fnode.node.resolve) {
      ...
      if (fnode.node.child.child || fnode.node.child.filter) {
        ...
        if (attrName) {
          const attrValue = path.node[attrName];
          const isASTNode = (v) => typeof v === "object" && v !== null && "type" in v;
          const wouldBeTraversed = isASTNode(attrValue) || Array.isArray(attrValue) && attrValue.length > 0 && isASTNode(attrValue[0]);
          if (!wouldBeTraversed) {
